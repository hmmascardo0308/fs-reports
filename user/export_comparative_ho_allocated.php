...
// export_comparative_ho_allocated.php
session_start();
...
// ── validate filters (mirrors comparative_report_original_ho_with_past_and_adjustment.php) ──
...
if (!$show_error && !empty($primary_period) && !empty($previous_period)) {
    ...
    if (!$show_error && !empty($third_period) && !is2025OrEarlier($third_period)) {
...
if ($show_error || !$valid_filters) {
    header('Content-Type: text/html');
    echo '<html><body><h2 style="color:red;">Export Error</h2><p>' . htmlspecialchars($error_message) . '</p>';
    echo '<p><a href="comparative_report_original_ho_with_past_and_adjustment.php">Return to Report</a></p>';
    echo '</body></html>';
    exit;
}
...
// ============================================================
// HARDCODED GL MAPPING FOR OLD GL CODES (identical to original)
// ============================================================
...
// ============================================================
// HARDCODED GL MAPPING FOR NEW GL CODES (identical to original)
// ============================================================
...
// ============================================================
// MIXED MODE LOOKUPS (identical to original)
// ============================================================
...
// ============================================================
// DETERMINE GL STRUCTURE TABLE (identical to original)
// ============================================================
...
// ============================================================
// HELPER: GET TOTAL FOR A SPECIFIC GL ID (identical to original)
// ============================================================
...
function get_gl_id_total(
    ...
    // If data is keyed directly by gl_id (manual_adjustment/past_transaction)
    if (isset($data[$gl_id])) {
...
function get_manual_adjustment_data_export(
    ...
    if ($gl_code_mode === 'old') {
        ...
        foreach ($gl_id_by_key as $key => $gl_id) {
            if ($gl_id !== '' && !isset($data[$gl_id]) && isset($raw_data[$gl_id]) && !array_key_exists($gl_id, $new_gl_mapping)) {
...
// ── compute rows (mirrors compute_table_rows_for_region_area exactly) ──────
...
function compute_table_rows_for_export(
    ...
    $fetch_period_data = function (string $period) use (
        ...
        if ($year >= 2026) {
            return get_manual_adjustment_data_export(
                $conn,
                $period,
                $gl_id_by_key,
                $gl_code_mode,
        ...
        return get_past_transaction_data_export(
            $conn,
            $period,
            $gl_id_by_key,
            $gl_code_mode,
    ...
    // Determine if third period is historical (2025 or earlier)
    $third_is_historical = !empty($third_period) && is2025OrEarlier($third_period);
    ...
    foreach ($gl_mapping as $key => $codes_detailed) {
        ...
        $previous_total = isset($previous_data[$current_gl_id]) ? (float)$previous_data[$current_gl_id] : 0.0;
        $third_total = isset($third_data[$current_gl_id]) ? (float)$third_data[$current_gl_id] : 0.0;
        ...
        if ((int)$sort_order === $inj3_sort_order && $current_gl_id === 'INJ-3') {
            ...
            // PRIMARY / PREVIOUS - Use the correct approach from original
            for ($inj_number = 1; $inj_number <= 49; $inj_number++) {
                ...
                $inj_previous = get_gl_id_total($inj_id, $gl_mapping, $gl_id_by_key, $previous_data, $prev_mode);
                
                // INJ-2: use display sign flip for Primary and Previous
                if ($inj_number === 2) {
            ...
            if ($third_is_historical) {
                // Historical (2025 and earlier): INJ-3 = INJ-1 - INJ-2 (no sign flip)
                $inj1_third = get_gl_id_total('INJ-1', $gl_mapping, $gl_id_by_key, $third_data, $t_mode);
                $inj2_third = get_gl_id_total('INJ-2', $gl_mapping, $gl_id_by_key, $third_data, $t_mode);
                $third_total = $inj1_third - $inj2_third;
            } else {
                // 2026+ logic with sign flip for INJ-2
                for ($inj_number = 1; $inj_number <= 49; $inj_number++) {
                    if ($inj_number === 3) continue;
                    
                    $inj_id = 'INJ-' . $inj_number;
                    $inj_third = get_gl_id_total($inj_id, $gl_mapping, $gl_id_by_key, $third_data, $t_mode);
        ...
        // INJ-2 DISPLAY SIGN FLIP (only for Primary and Previous, not Third)
        if ($is_inj2) {
            $primary_total = -$primary_total;
            $previous_total = -$previous_total;
            // $third_total is NOT flipped - matching original behavior
        }
    ...
    foreach ($grouped_rows as $sort_order => $rows) {
        ...
        if ((int)$sort_order >= $revenue_start && (int)$sort_order <= $revenue_end) {
        ...
        $percentage_third = (!empty($third_period) && $total_third_total != 0)
            ? ($inc_dec_third / abs($total_third_total)) * 100
            : (!empty($third_period) && $total_primary_total != 0 ? 100 : 0);
        ...
        if ($emit_total_revenues) {
            ...
            $pct_rev_third = (!empty($third_period) && $rev_tot_third != 0)
                ? ($inc_dec_rev_third / abs($rev_tot_third)) * 100
                : (!empty($third_period) && $rev_tot_p != 0 ? 100 : 0);
        ...
        if ((int)$sort_order === $cost_of_sales) {
            ...
            $pct_gp_third = (!empty($third_period) && $gp_tot_third != 0)
                ? ($inc_dec_gp_third / abs($gp_tot_third)) * 100
                : (!empty($third_period) && $gp_tot_p != 0 ? 100 : 0);
        ...
        if ((int)$sort_order === $sa_end) {
            ...
            $pct_sa_third = (!empty($third_period) && $sa_tot_third != 0)
                ? ($inc_dec_sa_third / abs($sa_tot_third)) * 100
                : (!empty($third_period) && $sa_tot_p != 0 ? 100 : 0);
            ...
            $pct_ebitda_third = (!empty($third_period) && $ebitda_tot_third != 0)
                ? ($inc_dec_ebitda_third / abs($ebitda_tot_third)) * 100
                : (!empty($third_period) && $ebitda_tot_p != 0 ? 100 : 0);
        ...
        if ((int)$sort_order === $depreciation) {
            ...
            $pct_ebit_third = (!empty($third_period) && $ebit_tot_third != 0)
                ? ($inc_dec_ebit_third / abs($ebit_tot_third)) * 100
                : (!empty($third_period) && $ebit_tot_p != 0 ? 100 : 0);
        ...
        if ((int)$sort_order === $interest) {
            ...
            $pct_ebt_third = (!empty($third_period) && $ebt_tot_third != 0)
                ? ($inc_dec_ebt_third / abs($ebt_tot_third)) * 100
                : (!empty($third_period) && $ebt_tot_p != 0 ? 100 : 0);
        ...
        if ((int)$sort_order === $tax) {
            ...
            $pct_net_third = (!empty($third_period) && $net_tot_third != 0)
                ? ($inc_dec_net_third / abs($net_tot_third)) * 100
                : (!empty($third_period) && $net_tot_p != 0 ? 100 : 0);
...
foreach ($data_rows as $item) {
    ...
    $is_numeric_sort = is_numeric($sort_order_val);
    $sort_num = $is_numeric_sort ? (int)$sort_order_val : 0;
    
    if ($is_sum) {
        $label = $sort_order_val;
        
        // Col A: numeric sort orders ≤ revenue_end are blank (description goes in B), specials go in A
        $sheet->setCellValue("A$row", ($is_numeric_sort && $sort_num <= $revenue_end) ? '' : $label);
        ...
        if (in_array($label, $highlight_labels, true)) {
            $bg = 'FFFFA973';
        } elseif ($is_numeric_sort && $sort_num % 2 != 0 && $sort_num <= $revenue_end) {
        ...
        // Borders
        $cost_of_sales = $ranges['cost_of_sales'];
        if ($is_numeric_sort && $sort_num == $cost_of_sales) {
        ...
        // Outline grouping for revenue summary rows (sort 1-revenue_end)
        if ($is_numeric_sort && $sort_num >= 1 && $sort_num <= $revenue_end) {
            $row++;
        ...
        // Detail row
        // Determine which sort_order to use for INJ-2 based on GL mode
        $inj2_sort_order = ($gl_code_mode === 'old') ? 17 : 18;
        
        if (
            (int)$item['sort_order'] === $inj2_sort_order &&
            in_array((int)$item['sub_order'], [3, 4, 5, 6])
        ) {
            $sheet->setCellValue("B$row", $item['gl_description'] ?? '');
        } else {
        ...
        if (abs($pct1) >= 1000) {
            ...
            $sheet->setCellValue("J$row", $pct1 / 100);
        }
        ...
        if (abs($pct2) >= 1000) {
            ...
            $sheet->setCellValue("L$row", $pct2 / 100);
        }
        ...
        // Detail rows for sort 1-revenue_end are grouped/hidden
        if ($is_numeric_sort && $sort_num >= 1 && $sort_num <= $revenue_end) {
            $sheet->getRowDimension($row)->setOutlineLevel(1)->setVisible(false);
        }
    }
