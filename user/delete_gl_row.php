...
try {
    ...
    $desc = null;
    $prefix = null;
    $desc_stmt = mysqli_prepare($conn, "SELECT description, gl_id FROM fs_reports.gl_codes_new WHERE id = ? LIMIT 1");
    if (!$desc_stmt) {
    ...
    // Delete the row
    $stmt = mysqli_prepare($conn, "DELETE FROM fs_reports.gl_codes_new WHERE id = ?");
    if (!$stmt) {
        throw new Exception('Prepare failed');
    }
