...
function compute_export_rows(
    ...
    if (!empty($primary_period)) {
        ...
        $sql = "SELECT gl_code, region, SUM(amount) as total_amount FROM fs_reports.fs_raw_data_summary $base_where AND transaction_year = ? AND transaction_month = ? AND gl_code IS NOT NULL AND gl_code != '' GROUP BY gl_code, region";
