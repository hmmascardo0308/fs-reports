...
try {
    ...
    $get_prefix = function($desc) use ($conn, &$prefixes) {
        ...
        $stmt = mysqli_prepare($conn, "SELECT gl_id FROM fs_reports.gl_codes_new WHERE description = ? AND gl_id IS NOT NULL AND gl_id != '' LIMIT 1");
    ...
    $update_stmt = mysqli_prepare($conn,
        "UPDATE fs_reports.gl_codes_new 
         SET sub_order = ?, gl_id = ?
    ...
    echo json_encode(['ok' => true, 'message' => 'Order and IDs updated successfully']);
