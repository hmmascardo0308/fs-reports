...
function getPreviewTotals(\mysqli $conn, array $years, mixed $month1, mixed $month2, mixed $month3) {
    $month1 = trim($month1 ?? '');
    $month2 = trim($month2 ?? '');
    $month3 = trim($month3 ?? '');
...
$data = getPreviewTotals($conn, $years, $month1, $month2, $month3);
...
if (isset($data['error'])) {
    ...
    // Create worksheet
    $worksheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, 'Financial Statement');
    $spreadsheet->addSheet($worksheet);
    $worksheet->setShowSummaryBelow(true);

    // Freeze columns A-D and rows 1-10
    ...
    if (file_exists($logoPath)) {
        ...
        $drawing->setCoordinates('F1'); // Place near description
    ...
    // Row 2: COMPARATIVE PROFIT & LOSS STATEMENT - w/ ALLOCATED HEAD OFFICE
    $worksheet->mergeCells("A$r:L$r");
    $worksheet->setCellValue("A$r", "COMPARATIVE PROFIT & LOSS STATEMENT - w/ ALLOCATED HEAD OFFICE");
...
$period2Label = $data['period2_label'];
$period3Label = $data['period3_label'];

$comparisonString = strtoupper("$currentLabel vs $period2Label vs $period3Label");
...
$r++;
    ...
    // Row 6: INCREASE/DECREASE
    $worksheet->mergeCells("I$r:L$r");
    $richText = new RichText();
    $increasePart = $richText->createTextRun('INCREASE/');
    $increasePart->getFont()->setBold(true);
    $decreasePart = $richText->createTextRun('DECREASE');
    $decreasePart->getFont()->setBold(true);
    $decreasePart->getFont()->setColor(new Color(Color::COLOR_RED));
    $worksheet->setCellValue("I$r", $richText);
    $worksheet->getStyle("I$r")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
    $worksheet->getStyle("I$r")->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('FFFFDBAC');
    $r++;

// Row 7: Dates and Variance Headers
$worksheet->setCellValue("E$r", strtoupper($currentLabel));
$worksheet->setCellValue("F$r", strtoupper($period2Label));
$worksheet->setCellValue("G$r", strtoupper($period3Label));

$worksheet->setCellValue("I$r", "PREVIOUS MONTH");
$worksheet->setCellValue("J$r", "%");
$worksheet->setCellValue("K$r", "PREVIOUS YEAR");
...
// Orangish background
$worksheet->getStyle("E$r:L$r")->getFill()
...
$r++;
    ...
    $worksheet->setCellValue("C$r", "Revenues");
    $worksheet->getStyle("A$r:N$r")->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('FFFF7F3A');
    $worksheet->getStyle("C$r")->getFont()->setBold(true);
    $r++;
    $tableStartRow = $r;
    ...
    $setPctValueAndStyle = function(int $rowNum, int $colIdx, float $pct, float $baseVal, float $diffVal, bool $isDeduction, bool $hasComparison) use ($worksheet, $cell, $setFontColor) {
    ...
    // Put label in second cell (column 1)
        $worksheet->setCellValue("C$r", $label);
    ...
    $writeLabelRow = function(string $label) use ($worksheet, &$r, $cell, $rowRange) {
        ...
        $base3 = $has_period3 ? $totals['p3'] : 0; // $cell originally has no comma

        // Only show % when a comparison period exists.
        ...
        // Keep variance cells zero when no comparison period is selected
            ...
            if (in_array($label, $labelTopBorderRows, true)) {
                $worksheet->getStyle($cell(5, $r) . ':' . $cell(14, $r))->getBorders()->getTop()
            ...
            if ($label === 'TOTAL NET INCOME/LOSS') {
                $worksheet->getStyle($cell(5, $r) . ':' . $cell(14, $r))->getBorders()->getTop()
                    ->setBorderStyle(Border::BORDER_THIN)
                    ->getColor()->setARGB(Color::COLOR_BLACK);
                $worksheet->getStyle($cell(5, $r) . ':' . $cell(14, $r))->getBorders()->getBottom()
        ...
        foreach ($glRows as $glRow) {
            ...
            if ($shouldRenderDetailRow($sortOrderVal)) {
                ...
                if ($sortOrderVal === 17 && $subOrderVal >= 3 && $subOrderVal <= 6) {
                    $worksheet->setCellValue("B$r", $comp);
                } else {
                    $worksheet->setCellValue("C$r", $comp);
                }
